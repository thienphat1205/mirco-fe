...
const TableBacklogList: React.FC<TableProps> = ({ search }) => {
  const { ticketsBacklog: { tickets = [], total = 0, isEmpty = false } = {} } =
    useTypedSelector((state) => state.ticket);
  const { setDataTicketReducer, getTicketsBacklog } = useActions();
...
export default TableBacklogList;
