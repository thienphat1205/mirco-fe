...
const TableArrearsList: React.FC<TableProps> = ({ search }) => {
  const { arrearsTickets: { tickets = [], total = 0, isEmpty = false } = {} } =
    useTypedSelector((state) => state.ticket);
  const { getArrearsTickets, setDataTicketReducer } = useActions();
...
export default TableArrearsList;
